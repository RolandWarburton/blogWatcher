...
const pageRouter = async (req, res) => {
	...
	const err =
		"_id field must be included in update request. Place string _id in body.filter._id";
	...
	switch (fieldName) {
		...
		case "source": {
			...
			if (
				!req.body.update.source[0].remote &&
				!req.body.update.source[0].url
			) {
				err +=
					"incorrect structure for source array. update.source must container an array of {url: String, remote: Bool}";
			}
		...
		case "__v": {
			...
			if (!req.body.update.__v) {
				err =
					"update.__v not provided correctly. { update.__v } must exist in update";
			}
